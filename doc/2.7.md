...
在我们的第一个测试中，我们专注解决代码内部的行为。在这个测试中，我们想检查 bug 的方式，就像用户浏览浏览器那样。
...
setup_test_environment()安装一个模板渲染器，允许我们检查 responses 的一些新增属性，例如 response.context，没有安装的都将不可用。注意，这个方法没有安装测试数据库，因此，以下将针对现有的数据库运行和输出可能略有不同，取决于您已经创建了什么问题。假如你在 setting.py 中设置的 TIME_ZONE 不正确，你可能会得到预期外的结果。假如你不记得有没有设置过，请检查再继续。
