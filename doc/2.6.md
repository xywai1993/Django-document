...
注意：我们的 vote()视图代码还有一些小问题。第一次从数据库获取 selected_choice 对象，计算新的投票值，然后保存到数据库中。假如两个用户访问你的网站并且恰好同时投票，这将会出现问题：这相同的值，让我们假设为 42，投票后为 43，然后，2 个用户的新值都计算为 43 并且保存，但是 44 才是正确的值。
